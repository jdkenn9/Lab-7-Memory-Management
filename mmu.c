...
void allocate_memory(list_t * freelist, list_t * alloclist, int pid, int blocksize, int policy) {
  ...
  if (list_is_in_by_size(freelist, blocksize)){
    greater = list_get_index_of_by_Size(freelist, blocksize);
  ...
  block_t* newblk = list_remove_at_index(freelist,greater);
  int end = newblk->end;
  newblk->pid = pid;
...
void deallocate_memory(list_t * alloclist, list_t * freelist, int pid, int policy) { 
    ...
    */
    ...
    if(list_is_in_by_pid(alloclist, pid)){
      remove = list_get_index_of_by_Pid(alloclist,pid);
      newblk = list_remove_at_index(alloclist, remove);
    }
    else{
    ...
    newblk->pid = 0;
    if(policy == 1){
    list_add_to_back(freelist, newblk);
    }
    if(policy == 2){
      list_add_ascending_by_blocksize(freelist, newblk);
    }
    if(policy == 3){
      list_add_descending_by_blocksize(freelist, newblk);
    }
}
